...
const verificationEmail = asyncHandler(async (req, res, next) => {
  ...
  if (user.verify) throw createError(401, "You have already verified. Login in to continue.");
...
const updatePassword = asyncHandler(async (req, res, next) => {
  ...
  if (!isMatch) throw createError(400, `Current password ${req.body.currentPassword} does't match`);
...
const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) throw createError(400, `User with email ${req.body.email} is not found`);
  ...
  try {
    ...
    res.status(200).send({ status: "success", message: "ResetPassword token Email sent" });
  } catch (error) {
    console.log(error);
...
const resetPassword = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).send({ status: "success", message: "Your Password has beed changed" });
});
